...
namespace Server
{
    ...
    {
        private static volatile Servant instanceServant;
        ...
        public static Servant Instance
        {
            get
            {
                if (instanceServant == null)
                {
                    lock (syncRoot)
                    {
                        if (instanceServant == null)
                            instanceServant = new Servant();
                    }
                }

                return instanceServant;
            }
        }
        ...
        public static void ReadCallback(IAsyncResult ar)
        {
            ...
            if (bytesRead > 0)
            {
                ...
                if (content.IndexOf("<EOF>") > -1)
                {
                    ...
                    byte[] plainText = objectCommand.DecodifyPackage(Encoding.ASCII.GetBytes(content.Replace("<EOF>", String.Empty)), Encoding.ASCII.GetString(privateKey));

                    if (plainText != null)
                        command.commBytes = plainText;
